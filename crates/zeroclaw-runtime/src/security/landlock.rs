...
impl LandlockSandbox {
    ...
    fn apply_restrictions(&self) -> std::io::Result<()> {
        ...
        {
            let workspace_fd =
                PathFd::new(workspace).map_err(io_other)?;
            ruleset = ruleset
        ...
        let tmp_fd =
            PathFd::new(Path::new("/tmp")).map_err(io_other)?;
        ruleset = ruleset
            .add_rule(PathBeneath::new(
                tmp_fd,
        ...
        let usr_fd =
            PathFd::new(Path::new("/usr")).map_err(io_other)?;
        ruleset = ruleset
            .add_rule(PathBeneath::new(
                usr_fd,
        ...
        let bin_fd =
            PathFd::new(Path::new("/bin")).map_err(io_other)?;
        ruleset = ruleset
            .add_rule(PathBeneath::new(
                bin_fd,
                AccessFs::ReadFile | AccessFs::ReadDir,
