...
channel-discord-approval-recorded = 你的决定已被记录。
...
turn-ingress-dropped = 未处理此请求：{ $reason }
...
cli-interactive-banner = 🦀 ZeroClaw 交互模式
cli-interactive-help-hint = 输入 /help 查看命令。
cli-interactive-quit-hint = 输入 /quit 退出。
cli-interactive-help-header = 可用命令：
cli-interactive-help-line-help = {"  "}/help             显示此帮助信息
cli-interactive-help-line-clear = {"  "}/clear /new       清除对话历史
cli-interactive-help-line-quit = {"  "}/quit /exit       退出交互模式
cli-interactive-help-line-think = {"  "}/think:<level>    设置推理深度（off|minimal|low|medium|high|max）
cli-interactive-clear-warning = 这将清除当前对话并删除所有会话记忆。
cli-interactive-clear-preserve = 核心记忆（长期事实/偏好）将被保留。
cli-interactive-continue-prompt = 继续？[y/N]
cli-interactive-cancelled = 已取消。
cli-interactive-cleared-with-count = 对话已清除（已移除 {$count} 条记忆条目）。
cli-interactive-cleared = 对话已清除。
cli-interactive-error-reading-input = 读取输入时出错：{ $error }
